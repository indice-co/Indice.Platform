...
internal class WorkflowCaseActionsService : ICaseActionsService
{
    ...
    public WorkflowCaseActionsService(
    ...
    public async ValueTask<CaseActions> GetUserActions(ClaimsPrincipal user, Guid caseId) {
        ...
        return user.IsAdmin()
            ? new CaseActions {
                ...
                CustomActions = customCaseActions.ToList()
            }
            : new CaseActions {
    ...
    private async Task<IEnumerable<CustomCaseAction>> GetCustomCaseActions(Guid caseId, IEnumerable<string> userRoles) {
        ...
        userRoles = userRoles.Concat(new[] {
            string.Empty
        });
