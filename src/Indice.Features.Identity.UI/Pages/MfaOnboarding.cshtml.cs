...
public abstract class BaseMfaOnboardingModel : BasePageModel
{
    ...
    public virtual async Task<IActionResult> OnGetAsync([FromQuery] string? returnUrl) {
        Input = View = new MfaOnboardingViewModel {
            AuthenticationMethods = await _authenticationMethodProvider.GetAllMethodsAsync(),
            ReturnUrl = returnUrl
        };
        return Page();
    }
    ...
    public virtual async Task<IActionResult> OnPostAsync([FromQuery] string? returnUrl) {
        ...
        TempData.Put(TempDataKey, new MfaOnboardingTempDataModel {
            SelectedAuthenticationMethod = Input.SelectedAuthenticationMethod
        });
