...
public class RegisterInputModelValidator : AbstractValidator<RegisterInputModel>
{
    ...
    public RegisterInputModelValidator(
        ...
        if (identityUiOptions.Value.RegisterRequireZoneInfo) {
            RuleFor(x => x.ZoneInfo).NotEmpty()
                .WithName(localizer["Timezone"])
                .Must(x => x is { } && ZoneInfo.ContainsKey(x))
                .WithMessage(localizer["The field '{PropertyName}' must be one of the provided values."]);
        }
