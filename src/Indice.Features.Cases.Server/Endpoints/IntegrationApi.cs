...
internal static class IntegrationApi
{
    public static IEndpointRouteBuilder MapIntegration(this IEndpointRouteBuilder routes) {
        ...
        // group.MapGet("test", IntegrationHandlers.Test)
        //     .AllowAnonymous();
        //
        // group.MapPost("test2", IntegrationHandlers.Test2)
        //     .AllowAnonymous();
        
        group.MapPost("create-workflow", IntegrationHandlers.CreateWorkflow)
            .AllowAnonymous();
        
        group.MapPost("create-rules", IntegrationHandlers.CreateRules)
            .AllowAnonymous();
        
        group.MapPost("run-workflow", IntegrationHandlers.RunWorkflow)
            .AllowAnonymous();
        ...
        group.MapGet("subscriptions", IntegrationHandlers.GetSubscriptionsByFilters)
            .WithName(nameof(IntegrationHandlers.GetSubscriptionsByFilters))
            .WithSummary("Get Subscriptions by filters.");
