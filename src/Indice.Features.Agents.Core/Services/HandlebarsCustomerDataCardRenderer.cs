...
public sealed class HandlebarsCustomerDataCardRenderer : ICustomerDataCardRenderer
{
    ...
    /// <inheritdoc/>
    public DataContent Render(CustomerState record) {
    ...
    private static Dictionary<string, object?> BuildModel(CustomerState record) {
    ...
    private static string DeriveTitle(CustomerState record) {
        ...
        return string.IsNullOrWhiteSpace(record.DataType) ? record.ReferenceId : $"{Humanize(record.DataType)} {record.ReferenceId}".Trim();
    }
