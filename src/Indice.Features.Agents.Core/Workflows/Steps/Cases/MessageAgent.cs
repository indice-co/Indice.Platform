...
/// </summary>
public sealed class MessageAgent : Executor<IntentOutput, RagPipelineOutput>
{
    ...
    /// <summary>Creates a new <see cref="MessageAgent"/>.</summary>
    public MessageAgent(AzureOpenAIClient openAIClient, IOptions<AgentsOptions> options,
        IOptions<ModelsOptions> models, IPromptTemplateRenderer prompts,
        UserClaimsAIContextProvider userClaimsProvider, IMcpToolsRegistry mcpToolsRegistry) : base("MessageAgent") {
    ...
    /// <inheritdoc/>
    public override async ValueTask<RagPipelineOutput> HandleAsync(IntentOutput envelope,
        IWorkflowContext context, CancellationToken cancellationToken = default) {

        var mcpTools = await _mcpToolsRegistry.GetToolsAsync("message", cancellationToken);

        var chatOptions = _models.BaseReasoningModelOptions.Clone();
        chatOptions.Instructions = _prompts.Render("MessageTools", new {
        ...
        var prompt = BuildPrompt(conversationState.Message.Text ?? string.Empty, []);
