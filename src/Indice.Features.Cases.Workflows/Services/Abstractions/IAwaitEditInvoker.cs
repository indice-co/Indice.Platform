...
internal interface IAwaitEditInvoker
{
    ...
    Task<IEnumerable<CollectedWorkflow>> DispatchWorkflowsAsync(Guid caseId, InvokeEditRequest request, CancellationToken cancellationToken = default);

    /// <summary>Executing a workflow will execute the workflow directly before returning.</summary>
    /// <param name="caseId">The Id of the case.</param>
    /// <param name="request">The edit request as dynamic json data.</param>
    /// <param name="cancellationToken">The cancellation token.</param>
    Task<IEnumerable<CollectedWorkflow>> ExecuteWorkflowsAsync(Guid caseId, InvokeEditRequest request, CancellationToken cancellationToken = default);
}