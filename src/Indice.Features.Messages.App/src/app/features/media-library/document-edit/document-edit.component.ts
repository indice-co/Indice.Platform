...
export class DocumentEditComponent implements OnInit {
  ...
  constructor(
    ...
    private _toaster: AppTranslatedToaster // updated
  ) { }
  ...
  public copyToClipboard(file: MediaFile): void {
    this._fileUtilitiesService.copyPathToClipboard(file.path)
      .then(() => {
        this._toaster.show(ToastType.Success, 'MediaLibrary.CopyPathTitle', 'MediaLibrary.CopyPathSuccessMessage', undefined, { path: file.path }); // localized single line
      })
      .catch((err) => {
        this._toaster.show(ToastType.Error, 'MediaLibrary.CopyPathErrorTitle', 'MediaLibrary.CopyPathErrorMessage'); // localized single line
      });
  }
}
