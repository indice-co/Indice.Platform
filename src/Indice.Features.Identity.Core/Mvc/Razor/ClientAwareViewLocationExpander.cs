...
namespace Indice.Features.Identity.Core.Mvc.Razor
{
    /// <summary>
    /// A view location expander that overrides view search locations when a client_id query string parameter exists in the URL.
    /// Suppose we have an AccountController with a Login view. The default view location is Views/Account/Login. 
    /// When a client with id <b>spa</b> exists in the query string, then a new view location takes precedence which is Views/Account/spa/Login.
    /// </summary>
