...
public interface IUserDeviceStore<TUser> where TUser : DbUser
{
    /// <summary>Returns an <see cref="IQueryable{Device}"/> collection of devices.</summary>
    public IQueryable<DbUserDevice> UserDevices { get; }
    ...
    Task<IdentityResult> CreateDeviceAsync(TUser user, DbUserDevice device, CancellationToken cancellationToken = default);
    ...
    Task<IList<DbUserDevice>> GetDevicesAsync(TUser user, UserDeviceListFilter filter = null, CancellationToken cancellationToken = default);
    ...
    Task<DbUserDevice> GetDeviceByIdAsync(TUser user, string deviceId, CancellationToken cancellationToken = default);
    /// <summary>Updates the given device. If the device does not exists, it is automatically created.</summary>
    /// <param name="user">The user instance.</param>
    /// <param name="device">The device to update.</param>
    /// <param name="cancellationToken">The <see cref="CancellationToken"/> used to propagate notifications that the operation should be canceled.</param>
    /// <returns>The <see cref="Task"/> that represents the asynchronous operation, containing the user device, if any.</returns>
    Task<IdentityResult> UpdateDeviceAsync(TUser user, DbUserDevice device, CancellationToken cancellationToken = default);
    ...
    Task RemoveDeviceAsync(TUser user, DbUserDevice device, CancellationToken cancellationToken = default);
