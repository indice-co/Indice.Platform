...
public class DefaultDevicePasswordHasher : IDevicePasswordHasher
{
    ...
    private readonly PasswordHasher<DbUser> _passwordHasher;
    ...
    public DefaultDevicePasswordHasher(PasswordHasher<DbUser> passwordHasher) {
        _passwordHasher = passwordHasher ?? throw new ArgumentNullException(nameof(passwordHasher));
    }

    /// <inheritdoc />
    public string HashPassword(DbUserDevice device, string password) => _passwordHasher.HashPassword(null, password);

    /// <inheritdoc />
    public PasswordVerificationResult VerifyHashedPassword(DbUserDevice device, string hashedPassword, string providedPassword) => _passwordHasher.VerifyHashedPassword(null, hashedPassword, providedPassword);
}
