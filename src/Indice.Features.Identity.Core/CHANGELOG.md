...
IF NOT EXISTS (
    SELECT 1
    FROM sys.columns 
    WHERE Name = N'PublicKeyId' AND Object_ID = Object_ID(N'[auth].[UserDevice]')
)
BEGIN
    ALTER TABLE [auth].[UserDevice]
...
1. **Config schema** new column for autogenerated claims

    ```sql
    ALTER TABLE [config].[ClaimType]
       ADD AutoGenerated BIT DEFAULT 0 NOT NULL
    GO
    update [config].[ClaimType] set Reserved = 1 where [Name] in (
    'birthdate'
    ,'email'
    ,'email_verified'
    ,'family_name' 
    ...
    ,'consent_commercial'
    ,'consent_commercialDate'
    ,'developer_totp')
    GO
    update [config].[ClaimType] set AutoGenerated = 1 where [Name] in (
    'email'
    ,'email_verified'
    ,'name'
    ,'phone_number' 
