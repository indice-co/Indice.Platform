...
internal class DbActivityLogEntryMap : IEntityTypeConfiguration<DbActivityLogEntry>
{
    public DbActivityLogEntryMap(string schema) {
        Schema = schema ?? throw new ArgumentNullException(nameof(schema));
    }

    public string Schema { get; }

    public void Configure(EntityTypeBuilder<DbActivityLogEntry> builder) {
        // Configure table name and schema.
        builder.ToTable("ActivityLog", Schema);
        ...
        builder.Property(x => x.HttpMethod).HasMaxLength(TextSizePresets.S16);
