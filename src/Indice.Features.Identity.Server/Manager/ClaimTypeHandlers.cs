...
internal static class ClaimTypeHandlers
{
    internal static async Task<Ok<ResultSet<ClaimTypeInfo>>> GetClaimTypes(
        ...
        if (filter?.Required is bool required) {
            query = query.Where(x => x.Required == required);
        }
        if (filter?.AutoGenerated is bool autoGenerated) {
            query = query.Where(x => x.AutoGenerated == autoGenerated);
        }
