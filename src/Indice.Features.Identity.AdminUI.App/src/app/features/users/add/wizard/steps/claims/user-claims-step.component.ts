  ...
  implements OnInit, OnDestroy
{
  ...
  public ngOnInit(): void {
    this._api
      .getClaimTypes(1, 2147483647, "name+", undefined)
      .subscribe((response: ClaimTypeInfoResultSet) => {
        this._selectedClaimsControl = this.data.form.controls["userClaims"];
        this.availableClaims = response.items.filter(
          (x) => !this._selectedClaimsControl.value.includes(x.name)
        );
        this.selectedClaims = response.items.filter((x) =>
          this._selectedClaimsControl.value.includes(x.name)
        );
      });
  }
