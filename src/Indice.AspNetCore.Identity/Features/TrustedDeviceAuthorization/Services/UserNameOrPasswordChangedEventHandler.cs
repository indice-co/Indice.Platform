...
namespace Indice.AspNetCore.Identity.TrustedDeviceAuthorization.Services
{
    internal class UserNameOrPasswordChangedEventHandler : IPlatformEventHandler<UserNameChangedEvent>, IPlatformEventHandler<PasswordChangedEvent>
    {
        ...
        public Task Handle(UserNameChangedEvent @event) => _userManager.SetDevicesRequirePasswordAsync(@event.User);

        public Task Handle(PasswordChangedEvent @event) => _userManager.SetDevicesRequirePasswordAsync(@event.User);
    }
}
