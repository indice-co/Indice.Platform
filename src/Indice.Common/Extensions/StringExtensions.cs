...
public static partial class StringExtensions
{
    ...
    [GeneratedRegex(@"(?<delimiter>[/\\-])?(?<word>[A-Z]?[a-z0-9.]+)[^A-Za-z0-9.]*")]
...
#else
    private static readonly Regex _wordsRegex = new(@"(?<delimiter>[/\\-])?(?<word>[A-Z]?[a-z0-9.]+)[^A-Za-z0-9.]*");
    private static readonly Regex _wordsAllCapsRegex = new(@"(?<delimiter>[/\\-])?(?<word>[A-Z][A-Z0-9.]*)[^A-Za-z0-9.]*");
    /// <summary>Match all parts of a sentence that start with one capital letter e.g. Net</summary>
