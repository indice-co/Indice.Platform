...
public class ClientIpRestrictionOptions
{
    ...
    /// <summary>Default Configuration key</summary>
    internal string ConfigurationSectionName { get; private set; } = DEFAULT_CONFIG_SECTION_KEY;

    /// <summary>Paths that are exluded from <see cref="Mappings"/>, optionally based on provided HTTP method.</summary>
    ...
    /// <summary>Map of route paths and client ips that will excluded from any restrictions.</summary>
    internal Dictionary<string, string> Mappings { get; } = new Dictionary<string, string>(StringComparer.InvariantCultureIgnoreCase);
    /// <summary>Name value pair for lists of safe ips (in byte array format) and their friendly names.</summary>
    internal Dictionary<string, byte[][]> IpAddressLists { get; } = new Dictionary<string, byte[][]>(StringComparer.InvariantCultureIgnoreCase);

    /// <summary>Status code to use when a client ip is dened access. Defaults to <see cref="HttpStatusCode.Forbidden"/></summary>
    ...
    /// <summary>Toggles feature. Defaults to false</summary>
    public bool Disabled { get; set; }

    /// <summary>Adds new list of ips under a given name.</summary>
    ...
    /// <summary>Excludes a mapped path, optionally based on the given HTTP method. If HTTP method is not specified, every request to this path will not be used by <see cref="ClientIpRestrictionMiddleware"/>.</summary>
