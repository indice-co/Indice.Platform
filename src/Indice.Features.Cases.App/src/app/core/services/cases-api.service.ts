...
export interface ICasesApiService {
     ...
     * @param caseId The Id of the case.
     * @param api_version (optional) 
     * @param body (optional) auditMeta.
     * @return No Content
     */
    assignCase(caseId: string, api_version?: string | undefined, body?: AuditMeta | undefined): Observable<void>;
    /**
    ...
    /**
     * Fetch the candidate Assignee for current case.
     * @param caseId (optional) The Id of the case.
     * @param api_version (optional) 
     * @return Success
     */
    getAssignees(caseId?: string | undefined, api_version?: string | undefined): Observable<Assignee[]>;
    /**
     * Fetch customers.
...
export class CasesApiService implements ICasesApiService {
    ...
    protected processGetDistinctCheckpointTypes(response: HttpResponseBase): Observable<CheckpointType[]> {
        ...
        return _observableOf<CheckpointType[]>(null as any);
    }
     ...
     */
    getAssignees(caseId?: string | undefined, api_version?: string | undefined): Observable<Assignee[]> {
        let url_ = this.baseUrl + "/api/manage/integrations/assignees?";
        if (caseId === null)
        ...
        return this.http.request("get", url_, options_).pipe(_observableMergeMap((response_ : any) => {
            return this.processGetAssignees(response_);
        })).pipe(_observableCatch((response_: any) => {
            if (response_ instanceof HttpResponseBase) {
                try {
                    return this.processGetAssignees(response_ as any);
                } catch (e) {
                    return _observableThrow(e) as any as Observable<Assignee[]>;
                }
            } else
                return _observableThrow(response_) as any as Observable<Assignee[]>;
        }));
    }

    protected processGetAssignees(response: HttpResponseBase): Observable<Assignee[]> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
            (response as any).error instanceof Blob ? (response as any).error : undefined;
        ...
        if (status === 400) {
            ...
            if (Array.isArray(resultData200)) {
                result200 = [] as any;
                for (let item of resultData200)
                    result200!.push(Assignee.fromJS(item));
            }
            else {
                result200 = <any>null;
            }
        ...
        return _observableOf<Assignee[]>(null as any);
    }
