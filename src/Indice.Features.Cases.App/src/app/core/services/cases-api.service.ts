...
export interface ICasesApiService {
    ...
    /**
     * Gets the ids of the correlated cases for a case.
     * @param caseId The id of the case.
     * @param api_version (optional)
     * @return OK
     */
    getCorrelatedCases(caseId: string, api_version?: string | undefined): Observable<string[]>;
    /**
...
export class CasesApiService implements ICasesApiService {
     ...
     */
    getCorrelatedCases(caseId: string, api_version?: string | undefined): Observable<string[]> {
        let url_ = this.baseUrl + "/api/manage/cases/{caseId}/correlated-case-ids?";
        if (caseId === undefined || caseId === null)
            throw new Error("The parameter 'caseId' must be defined.");
        url_ = url_.replace("{caseId}", encodeURIComponent("" + caseId));
        if (api_version === null)
        ...
        return this.http.request("get", url_, options_).pipe(_observableMergeMap((response_ : any) => {
            return this.processGetCorrelatedCases(response_);
        })).pipe(_observableCatch((response_: any) => {
            if (response_ instanceof HttpResponseBase) {
                try {
                    return this.processGetCorrelatedCases(response_ as any);
                } catch (e) {
                    return _observableThrow(e) as any as Observable<string[]>;
                }
            } else
    ...
    protected processGetCorrelatedCases(response: HttpResponseBase): Observable<string[]> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
            (response as any).error instanceof Blob ? (response as any).error : undefined;
