...
{
    [Trait("Services", "LayoutTranslationsServiceTests")]
    public class JsonTranslationServiceTest
    {
        ...
        public void Translate_EmptyLayout_ReturnSourceJson() {
            var translations = _service.Translate(string.Empty, "translations", "el");
            Assert.Equal(string.Empty, translations);
        }

        [Fact]
        public void Translate_EmptyTranslations_ReturnSourceJson() {
            var translations = _service.Translate("source", string.Empty, "el");
            Assert.Equal("source", translations);
        }

        [Fact]
        public void Translate_EmptyLocale_Throws() {
            Assert.Throws<ArgumentNullException>(() => _service.Translate("source", "translations", string.Empty));
        }
        ...
        public void Translate_IntegerValue_ValueNotChanged() {
            ...
            var translations = _service.Translate(layout, Translations, "en");
            Assert.Equal("[{\"section\":\"test\",\"someNumber\":32}]", translations);
        }
        ...
        public void Translate_DecimalValue_ValueNotChanged() {
            ...
            var translations = _service.Translate(layout, Translations, "en");
            Assert.Equal("[{\"section\":\"test\",\"someNumber\":32.31}]", translations);
        }
        ...
        public void Translate_StringValueWithTranslatedProperty_Translated() {
            ...
            var translations = _service.Translate(layout, Translations, "en");
            Assert.Equal("[{\"section\":\"test\",\"title\":\"Country code\"}]", translations);
        }
        ...
        public void Translate_StringValueWithNOtTranslatedProperty_Translated() {
            ...
            var translations = _service.Translate(layout, Translations, "en");
            Assert.NotEqual("[{\"section\":\"test\",\"something\":\"Country code\"}]", translations);
        }

        [Fact]
        public void Translate_ComplexLayout_El_NotTranslated() {
            var translations = _service.Translate(SourceJson, Translations, "el");
            Assert.Equal(SourceJson, translations);
        }

        [Fact]
        public void Translate_ComplexLayout_En_Translated() {
            var translations = _service.Translate(SourceJson, Translations, "en");

            Assert.False(translations.Contains("Θέλω να επικαιροποιήσω το Σταθερό Τηλέφωνο μου"));
            Assert.True(translations.Contains("I want to validate my home phone"));
        }
    }
}
